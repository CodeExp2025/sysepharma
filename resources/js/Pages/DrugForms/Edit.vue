...
const form = useForm({
    name: props.drugForm.name,
    description: props.drugForm.description || '',
    is_active: props.drugForm.is_active,
});

const submit = () => {
    form.put(route('drug-forms.update', props.drugForm.uuid));
};
</script>

<template>
    <AppLayout title="Modifier la forme pharmaceutique">
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="font-semibold text-2xl text-gray-800 leading-tight">
                        Modifier la forme pharmaceutique
                    </h2>
                    <p class="text-sm text-gray-600 mt-1">
                        Modifiez les informations de la forme
                    </p>
                </div>
                <Link
                ...
                <form @submit.prevent="submit" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows="3"
                            placeholder="Description optionnelle de la forme pharmaceutique..."
                            class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
