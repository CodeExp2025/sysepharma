...
const form = useForm({
    category_id: props.drug.category_id,
    name: props.drug.name,
    effet_s_med: props.drug.effet_s_med,
    dosage_med: props.drug.dosage_med,
    form_med: props.drug.form_med,
    prix_med: props.drug.prix_med,
...
<template>
    <AppLayout title="Modifier Médicament">
        <template #header>
        ...
        <div class="py-8">
            <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="mb-6 bg-blue-50 border border-blue-100 rounded-lg p-4">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 flex-1">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-medium text-blue-800">Informations du médicament</p>
                                    <p class="text-xs text-blue-700 mt-1">
                                        Créé le {{ new Date(drug.created_at).toLocaleDateString('fr-FR') }}
                                        <span v-if="drug.updated_at !== drug.created_at">
                                            • Modifié le {{ new Date(drug.updated_at).toLocaleDateString('fr-FR') }}
                                        </span>
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="p-8">
                            ...
                            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                                <button 
                                    type="button"
                                    @click="deleteDrug"
                                    class="inline-flex items-center px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 font-medium rounded-lg transition-colors border border-red-200"
                                >
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Change History (Optional enhancement) -->
                <div class="mt-6 bg-gray-50 border border-gray-200 rounded-lg p-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h4 class="text-sm font-medium text-gray-800 mb-1">Historique</h4>
                            <p class="text-xs text-gray-600">
                                Dernière modification : {{ new Date(drug.updated_at).toLocaleString('fr-FR') }}
                            </p>
                        </div>
                    </div>
                </div>
